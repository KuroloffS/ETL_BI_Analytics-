...
# Функция для записи логов в таблицу log_manifest
def log_to_db(filepath, dtmod, dtflow, file_ext, filename, rowsqty, filesize_mb, message, iserror):
    try:
        # Подключение к базе данных PostgreSQL
        conn = psycopg2.connect(
            dbname=db_name,
        ...
        # Запрос на вставку лога
        insert_log_query = """
        INSERT INTO log_manifest (filepath, dtmod, dtflow, file_ext, filename, rowsqty, filesize_kb, message, iserror)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);
        """
        cursor.execute(insert_log_query, (filepath, dtmod, dtflow, file_ext, filename, rowsqty, filesize_mb, message, iserror))
        conn.commit()
        cursor.close()
        conn.close()
    except Exception as e:
        print(f"Ошибка записи лога в базу данных: {e}")

# Функция для обработки одного Excel файла
def process_excel_file(file_path):
    # Дата создания файла
    dtmod = datetime.fromtimestamp(os.path.getmtime(file_path)).strftime('%Y-%m-%d %H:%M:%S')
    dtflow = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    file_ext = os.path.splitext(file_path)[1][1:]
    filename = os.path.basename(file_path)
    rowsqty = 0
    filesize_mb = os.path.getsize(file_path) // (1024)
    message = ""
    iserror = False
    ...
    try:
        ...
        if df.empty:
            message = f"Файл {file_path} пуст или не содержит данных."
            iserror = True
        else:
            ...
            if not set(df.columns).issubset(set(original_names)):
                message = f"Файл {file_path} содержит несоответствующие поля и будет пропущен."
                iserror = True
            else:
                ...
                df['dtflow'] = dtflow
                df['dt_file_create'] = dtmod
                df['file_name'] = filename
                df['file_path'] = os.stat(file_path)
                ...
                write_to_db(df, dtmod, filename)
                message = f"Данные из файла {file_path} успешно обработаны."
    except Exception as e:
        message = f"Ошибка при обработке файла {file_path}: {e}"
        iserror = True

    # Запись лога в базу данных
    log_to_db(file_path, dtmod, dtflow, file_ext, filename, rowsqty, filesize_mb, message, iserror)
