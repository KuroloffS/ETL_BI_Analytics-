...
def write_to_db(df, dtcreate, file_name):
    try:
        ...
        INSERT INTO manifest ({columns}) VALUES ({placeholders});
        """).format(
        ...
        cursor.close()
        conn.close()
        print(f"Данные из файла {file_name} успешно записаны в базу данных.")
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.xlsx') and file.startswith('Manifest'):
                ...
                processed_files += 1
                print(f"Успешно обработан файл: {file_path}")
            else:
                print(f"Файл {file} не соответствует требованиям и будет пропущен.")
    return processed_files

# Обработка файлов в папке manifest
processed_files = process_directory(manifest_dir)
